                          ...
                          <div class="lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center text-center">
                            ...
                            <div>
                              @if (Auth::id())
                                ...
                                @endif
                              @endif
                              <button onclick="location.href='{{ route('user.items.showMap', $post->id )}}'" class="inline-flex text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded-lg text-sm" style="margin-bottom: 10px;">詳細Map</button>
                              @if (Auth::id())
                                @if($likeCheck === false)
                                <form action="{{ route('user.items.like',$post->id)}}" method="POST" style="margin-bottom: 10px;">
                                    @csrf
                                    <input type="submit" value="いいね" class="inline-flex text-white bg-red-600 border-0 py-2 px-6 focus:outline-none hover:bg-red-300 rounded-lg text-sm">
                                </form>
                                @else
                                <form action="{{ route('user.items.unlike',$post->id)}}" method="POST" style="margin-bottom: 10px;">
                                    @csrf
                                    <input type="submit" value="いいねを取り消す" class="inline-flex text-white bg-red-600 border-0 py-2 px-6 focus:outline-none hover:bg-red-300 rounded-lg text-sm">
                                </form>
                                @endif
                              @endif
                              ...
                              <a data-url="https://xs808261.xsrv.jp/public/" href="https://twitter.com/share" class="twitter-share-button" data-lang="ja" data-count="vertical" data-dnt="true" target="_blank">
