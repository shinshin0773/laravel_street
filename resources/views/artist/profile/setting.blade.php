        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="post" action="{{ route('artist.profile.settingUpdate')}}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="p-2 w-1/2 mx-auto">
                            <div class="relative">
                                <label for="information" class="leading-7 text-sm text-gray-600">Email ※必須</label>
                                <input type="text" id="email" name="email" value="{{ $profile->email }}" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                            </div>
                        </div>
                        <div class="p-2 w-1/2 mx-auto">
                            <div class="relative">
                                <label for="sns_account" class="leading-7 text-sm text-gray-600">Payment_URL ※PayPalの決済URLを記載してください</label>
                                <input type="text" id="payment_url" name="payment_url" value="{{ $profile->payment_url }}" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                            </div>
                        </div>
